								...
								<c:if test="${board.battachdata != null}">
									...
									<!-- 
										src의 속성값은 완전한 응답 HTTP가 되어야 함
											1) 서버의 정적을 요청해서 응답을 받는 경우 예) photo1.jsp
											2) 요청 경로를 이용해서 컨트롤러에서 생성한 응답을 생성하는 경우 예) filedownload2?bno=${board.bno}
										만약에 데이터를 직접 넣어야할 경우
										src="data:MIME;base64, base64로 인코딩된 데이터"
									-->
									<img src="data:${board.battachtype};base64, ${base64Img}" height="100"/>
									<%-- <img src="filedownload2?bno=${board.bno}" height="100"/> --%>
								</c:if>
							</span> <br/>
						</p>
					</c:if>
